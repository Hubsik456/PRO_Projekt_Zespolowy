...
def init(język):
    ...
    if OS.system("pybabel extract -F babel.cfg -k _l -o messages.pot ."):
        raise RuntimeError(
            "Polecenie 'init' wygenerowało błąd podczas tworzenia nowego języka."
        )

    if OS.system("pybabel init -i messages.pot -d Aplikacja/Tłumaczenia -l " + język):
        raise RuntimeError(
            f"Polecenie 'init' wygenerowało błąd podczas dodawania języla '{język}'."
        )
...
def update():
    ...
    if OS.system("pybabel extract -F babel.cfg -k _l -o messages.pot ."):
        raise RuntimeError(
            "Polecenie 'update' wygenerowało błąd podczas wyciągania tłumaczeń."
        )

    if OS.system("pybabel update -i messages.pot -d Aplikacja/Tłumaczenia"):
        raise RuntimeError(
            "Polecenie 'update' wygenerowało błąd podczas aktualizacji tłumaczeń."
        )
...
def compile():
    ...
    if OS.system("pybabel compile -d Aplikacja/translations"):
        raise RuntimeError(
            "Polecenie 'compile' wygenerowało błąd podczas kompilowania tłumaczeń."
        )
